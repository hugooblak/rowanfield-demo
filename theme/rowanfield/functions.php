<?php
/**
 * Rowanfield theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Rowanfield Leads" plugin, so leads survive a theme change.
 *
 * @package Rowanfield
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'rowanfield', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'rowanfield' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'rowanfield' ) ) );

		register_block_pattern_category( 'rowanfield-sections', array( 'label' => __( 'Rowanfield sections', 'rowanfield' ) ) );
		register_block_pattern_category( 'rowanfield-pages', array( 'label' => __( 'Rowanfield page layouts', 'rowanfield' ) ) );
	}
);
...
add_action(
	...
	function () {
		...
		$data = array(
			...
			'address'      => array(
				'@type'           => 'PostalAddress',
				'streetAddress'   => '120 Example Road',
				'addressLocality' => 'Rowan Valley',
				'addressRegion'   => 'OR',
				'postalCode'      => '97000',
				'addressCountry'  => 'US',
			),
			'openingHours' => array( 'Mo-Fr 07:00-19:00', 'Sa 08:00-14:00' ),
			'areaServed'   => array( 'Rowan Valley', 'Kestrel Flats', 'Pinewood Ridge' ),
		);
