...
// Prefill from the URL: ?zip=97001 (from the short form) and ?service=orchard (from service pages).
...
if ( 'start' === $npl_variant ) :
	...
	$npl_button     = ! empty( $attributes['buttonText'] ) ? $attributes['buttonText'] : __( 'Start my hive plan', 'rowanfield-leads' );
	?>
	<div <?php echo $npl_wrapper; // phpcs:ignore WordPress.Security.EscapeOutput ?>>
		<form class="npl-start" method="get" action="<?php echo esc_url( $npl_action ); ?>" novalidate data-npl-start>
			<label class="npl-label" for="<?php echo esc_attr( $npl_uid ); ?>-zip"><?php esc_html_e( 'Your ZIP code', 'rowanfield-leads' ); ?></label>
			...
			<p class="npl-fineprint"><?php esc_html_e( 'Step 1 of 4. Your details are never sold or shared.', 'rowanfield-leads' ); ?></p>
		</form>
	</div>
	<?php
	return;
...
$npl_steps = array(
	1 => __( 'Where is the block?', 'rowanfield-leads' ),
	2 => __( 'What do you need?', 'rowanfield-leads' ),
	3 => __( 'When does the bloom open?', 'rowanfield-leads' ),
	4 => __( 'Where should we send the plan?', 'rowanfield-leads' ),
);
?>
	...
	<form class="npl-steps" method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" novalidate data-npl-steps data-total="4" data-start="<?php echo (int) $npl_start; ?>">
		...
		<?php if ( $npl_errors ) : ?>
			<div class="npl-summary" role="alert" tabindex="-1" data-npl-summary>
				<p><strong><?php esc_html_e( 'Please check these answers:', 'rowanfield-leads' ); ?></strong></p>
				<ul>
		...
		<div class="npl-progress" data-npl-progress hidden>
			<p class="npl-progress-text" aria-live="polite" data-npl-progress-text><?php echo esc_html( sprintf( /* translators: %d: step number */ __( 'Step %d of 4', 'rowanfield-leads' ), $npl_start ) ); ?></p>
		...
		<fieldset class="npl-step" data-step="1" data-name="zip" tabindex="-1">
			...
			<label class="npl-label" for="<?php echo esc_attr( $npl_uid ); ?>-zip"><?php esc_html_e( 'ZIP code', 'rowanfield-leads' ); ?></label>
		...
		foreach ( array( 2 => 'service', 3 => 'timeline' ) as $npl_n => $npl_field ) :
			...
			<fieldset class="npl-step" data-step="<?php echo (int) $npl_n; ?>" data-name="<?php echo esc_attr( $npl_field ); ?>" tabindex="-1" id="<?php echo esc_attr( $npl_uid . '-' . $npl_field ); ?>" aria-describedby="<?php echo esc_attr( $npl_uid . '-' . $npl_field ); ?>-err"<?php echo isset( $npl_errors[ $npl_field ] ) ? ' aria-invalid="true"' : ''; ?>>
				...
				<p class="npl-hint npl-auto-hint"><?php esc_html_e( 'Choosing an answer takes you to the next question.', 'rowanfield-leads' ); ?></p>
		...
		<fieldset class="npl-step" data-step="4" data-name="contact" tabindex="-1">
			...
			<?php
			$npl_contact = array(
				'name'  => array( __( 'First name', 'rowanfield-leads' ), 'text', 'given-name', '' ),
				'phone' => array( __( 'Phone', 'rowanfield-leads' ), 'tel', 'tel-national', __( 'So we can check the block details.', 'rowanfield-leads' ) ),
				'email' => array( __( 'Email', 'rowanfield-leads' ), 'email', 'email', __( 'Your hive plan is sent here.', 'rowanfield-leads' ) ),
			);
		...
		<div class="npl-nav">
			<button type="button" class="npl-back" data-npl-back hidden><span aria-hidden="true">←</span> <?php esc_html_e( 'Back', 'rowanfield-leads' ); ?></button>
			<button type="button" class="npl-btn wp-element-button" data-npl-next hidden><?php esc_html_e( 'Next', 'rowanfield-leads' ); ?> <span aria-hidden="true">→</span></button>
			<button type="submit" class="npl-btn wp-element-button" data-npl-submit><?php esc_html_e( 'Get my hive plan', 'rowanfield-leads' ); ?></button>
		</div>
