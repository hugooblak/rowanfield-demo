		...
		register_post_type(
			...
			array(
				'labels'          => array(
					...
					'all_items'     => __( 'All leads', 'rowanfield-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'rowanfield-leads' ),
			'npl_phone'    => __( 'Phone', 'rowanfield-leads' ),
			'npl_service'  => __( 'Needs', 'rowanfield-leads' ),
			'npl_timeline' => __( 'When', 'rowanfield-leads' ),
			'npl_zip'      => __( 'ZIP', 'rowanfield-leads' ),
			'npl_source'   => __( 'Source', 'rowanfield-leads' ),
			'npl_status'   => __( 'Status', 'rowanfield-leads' ),
			'date'         => __( 'Received', 'rowanfield-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'rowanfield-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'rowanfield-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Follow-up status', 'rowanfield-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'rowanfield-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'rowanfield-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'Email', 'rowanfield-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'rowanfield-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'rowanfield-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'rowanfield-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'rowanfield-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Consent given', 'rowanfield-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'rowanfield-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
