...
add_filter(
	...
	function ( $exporters ) {
		$exporters['rowanfield-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'rowanfield-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'rowanfield-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'rowanfield-leads',
						'group_label' => __( 'Quote requests', 'rowanfield-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['rowanfield-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'rowanfield-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
