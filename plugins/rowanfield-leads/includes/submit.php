...
function npl_validate( $in ) {
	...
	if ( 5 !== strlen( $v['zip'] ) ) {
		$errors['zip'] = __( 'Enter your 5-digit ZIP code.', 'rowanfield-leads' );
	}
	if ( ! isset( $choices['service'][ $v['service'] ] ) ) {
		$errors['service'] = __( 'Choose what you need help with.', 'rowanfield-leads' );
	}
	if ( ! isset( $choices['timeline'][ $v['timeline'] ] ) ) {
		$errors['timeline'] = __( 'Choose when you would like the work done.', 'rowanfield-leads' );
	}
	if ( strlen( $v['name'] ) < 2 ) {
		$errors['name'] = __( 'Enter your first name.', 'rowanfield-leads' );
	}
	...
	if ( 10 !== strlen( $digits ) ) {
		$errors['phone'] = __( 'Enter a 10-digit phone number, like 555 010 0142.', 'rowanfield-leads' );
	} else {
		$v['phone'] = sprintf( '(%s) %s-%s', substr( $digits, 0, 3 ), substr( $digits, 3, 3 ), substr( $digits, 6 ) );
	}
	if ( ! is_email( $v['email'] ) ) {
		$errors['email'] = __( 'Enter an email address, like name@example.com.', 'rowanfield-leads' );
	}
...
function npl_handle_submit() {
	...
	if ( is_wp_error( $lead_id ) ) {
		wp_die( esc_html__( 'Sorry, something went wrong. Please call us on (555) 010-0142.', 'rowanfield-leads' ), 500 );
	}
